...
sudo ip link add veth22 type veth peer name veth22-peer # mongodb_n2
...
docker exec ovs ip link set veth21 up # bring up interface connected to edge_server_n2
docker exec ovs ip link set veth22 up # bring up interface connected to mongodb_n2
docker exec ovs ip link set veth23 up # bring up interface connected to router LAN side
...
docker run -dit --name edge_server_n2 --network none ubuntu-host

# Review as each network will have its own mongodb shard
# Load MongoDB env-file if present (to reuse init creds)
echo "Starting MongoDB shard member container mongodb_n2..."
docker run -dit --name mongodb_n2 --network none \
  --no-healthcheck \
  -v mongodb_n2-data:/data/db ubuntu-mongodb mongod \
...
PID_MONGO=$(docker inspect -f '{{.State.Pid}}' mongodb_n2)
...
# Configure mongodb_n2 container
...
sudo nsenter -t $PID_MONGO -n ip addr add 10.0.1.3/24 dev eth0
