#!/usr/bin/env python3
"""Prepare and attest the isolated RQ3 QoE worktree/tag.

The frozen base is the final robustness attestation commit 852a8a3; the QoE
family adds NO runtime source changes (fault knobs and arm env deltas are
reused byte-identical from the robustness family). This script hashes the same
protected runtime surface as the robustness prepare_tag so the attestation is
directly comparable.
"""
...
def main() -> int:
    ...
    manifest_path = (args.manifest or worktree / ".rq3qoe_frozen_manifest.json").resolve()
