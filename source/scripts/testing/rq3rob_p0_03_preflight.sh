#!/usr/bin/env bash
# Static checks for the isolated RQ3 robustness worktree.
set -euo pipefail
...
[[ -f "$MANIFEST" ]] || die "missing robustness manifest: $MANIFEST"
...
[[ -f "$REPO_ROOT/source/scripts/testing/rq3rob_p1_01_launch_run.sh" ]] || die "missing launcher"
...
grep -q 'docker_restart' "$REPO_ROOT/source/scripts/testing/fault_injector.py" \
    || die "fault injector has no docker_restart action"
for env in rq3rob_event_only rq3rob_loss_all rq3rob_loss_alt; do
...
print("RQ3 robustness static manifest checks: PASS")
PY

bash -n "$SCRIPT_DIR/rq3rob_p1_01_launch_run.sh"
python3 -m py_compile \
    "$SCRIPT_DIR/rq3rob_p0_01_prepare_tag.py" \
    "$SCRIPT_DIR/rq3rob_p0_02_analyzer_selftest.py" \
    "$SCRIPT_DIR/analysis/rq3/readiness_robustness.py"

echo "RQ3 robustness preflight preparation checks: PASS"
