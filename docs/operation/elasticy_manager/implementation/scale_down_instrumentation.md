...
makes it impossible to tell â€” from logs alone â€” whether scale-down never fired
...
- Observability surface: new DEBUG lines become the primary artifact consumed
  by the scale-down audit CLI defined in the testing plan
    ([`analysis_toolchain.md`](../../testing/analysis_toolchain.md)'s
...
Add a `prev_armed` flag per tier to detect the `False â†’ True` edge transition.
...
def evaluate_scale_down_compute(self, ds: DomainSummary) -> bool:
    if ds.avg_time_proc_ms > _SCALE_DOWN_PROC_TIMEOUT_CEILING_MS:
        logger.debug(
            "[scale-down] compute eval: proc=%.1f exceeds ceiling (%.0f) â€” window skipped",
...
def evaluate_scale_down_storage(self, ds: DomainSummary) -> bool:
    if ds.avg_time_db_ms > _SCALE_DOWN_DB_TIMEOUT_CEILING_MS:
        logger.debug(
            "[scale-down] storage eval: db=%.1f exceeds ceiling (%.0f) â€” window skipped",
...
[scale-down] <tier> eval: <metric>=<f> exceeds ceiling (<f>) â€” window skipped
...
`False` **without appending to the sliding deque** â€” the window is
...
timeout ceiling (Z) â€” skipping window`). Any external log consumer that
...
_RE_DOWN_CEILING = re.compile(
    r"\[scale-down\] (compute|storage) eval: "
    r"(proc|db)=([\d.]+) exceeds ceiling \(([\d.]+)\) â€” window skipped"
)
...
1. Running a controller at `LOG_LEVEL=DEBUG` produces exactly one `[scale-down]
   <tier> eval:` line per tier per telemetry window â€” no more, no less.
...
| Risk | Mitigation |
|---|---|
| DEBUG volume floods logs | ~1 line per tier per 10 s window per LAN â‰ˆ 720 lines/hour/LAN. Acceptable; gated by `LOG_LEVEL`. |
...
## Overview file changes â€” `elasticity_overview.md`
...
### 1. Scale-Down Sliding Window â€” add an instrumentation paragraph
...
### 2. Anti-Thrashing Mechanisms â€” no change to the table
...
### 3. New section â€” "Implementation Plans"
...
- [`implementation/plans/metric_drivers_investigation_plan.md`](implementation/plans/metric_drivers_investigation_plan.md)
  â€” umbrella investigation into what actually drives CPU / T_db / T_proc.
- [`implementation/scale_down_instrumentation.md`](implementation/scale_down_instrumentation.md)
  â€” DEBUG/INFO observability for the scale-down decision path.
- [`implementation/scaling_threshold_tuning_and_caps.md`](implementation/scaling_threshold_tuning_and_caps.md)
  â€” threshold tuning and hard-cap rationale (existing).
```

## Cross-references

- Umbrella investigation: [`./plans/metric_drivers_investigation_plan.md`](./plans/metric_drivers_investigation_plan.md)
- Log consumer: [`../../testing/analysis_toolchain.md`](../../testing/analysis_toolchain.md) (`cli_scale_down`)
