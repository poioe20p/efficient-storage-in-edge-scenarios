...
way to interpret a run is to eyeball the CSVs; this plan provides:
...
degrade gracefully when missing — the affected charts are skipped with a
warning.
...
| New | `source/scripts/testing/analysis/loader.py` | `load_run(path) → Run` |
...
CSVs on the same axis must convert explicitly — the 1000× mismatch is
...
# In main() — open a second CSV alongside the existing one:
...
**Acceptance:** after a ≥ 60 s collection, `per_node_stats.csv` exists with
row count ≈ Σ(active containers) × windows.
...
- `phases_snapshot.json` must contain `cross_region_ratio` per phase — it is
...
updated, so a ceiling-skipped window must be excluded from hit counts —
...
`T_db_write ~ a + b·storage_count + c·cross_region_ratio`.
...
If `b_storage_count > 0` with meaningful R², **adding storage nodes makes
writes slower** — this is the primary falsifiable claim of the parent
investigation.
...
- `summary.md` (appended by every CLI — one section per tool)

## Acceptance

1. Fresh run produces `per_node_stats.csv` with row count ≈ Σ(containers) × windows.
...
4. `cli_tdb_drivers` prints a table like
   `T_db_write ≈ 12.4 + 38.1·storage_count + 95.2·cross_region_ratio  (R²=0.71)`.
...
| Risk | Mitigation |
|---|---|
| `per_node_stats.csv` grows large on long runs | 10 nodes × 360 windows × 10 s window ≈ 3600 rows/run — trivial. Split per phase later if > 50 MB. |
...
## Overview file changes — `testing_overview.md`
...
### 1. Architecture diagram — add analysis branch
...
```
  ...
  metrics/resource_stats.csv          ←── domain windowed stats (existing + new columns)
  metrics/per_node_stats.csv          ←── per-container windowed stats (NEW)
  metrics/controller_lan{1,2}.log     ←── includes scale-down DEBUG lines
         │
         ▼
...
### 2. New Components section — "5. Run Analysis Toolchain"
...
### 3. Execution Order — add a post-run step
...
### 4. File Layout — add the analysis package and new CSV
...
Under `docs/operation/testing/` add the new plan entry:

```
├── analysis_toolchain_plan.md       ← this plan
```
