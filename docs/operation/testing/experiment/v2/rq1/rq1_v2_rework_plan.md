...
- **🛠️ G2 RETUNE (2026-08-04, root-caused collapse → Option A).** The true open-loop G2 (`20260804_165925_rq1_delivery_ep_calib2`) collapsed: lan1 timeout 87.7%, lan2 68.2%, dropped 3.1%/15.5%, lan1 dyn2 OOM-killed + base server restarted, lan1 telemetry silent from w54 → only 14 overload windows detected. **Root cause (not an independent lan1 bug):**
  ...
  **Per-arm scale-down arming (gate f) — A/B/C ✅ PASS, 2026-08-05.** Arm A `rq1_g2_rate12_mix_ec25` (4 real removals), Arm B `rq1_g2_armB_arm` (real storage removals in recovery_gap). **Arm C `rq1_g2_armC_arm` FIRST RUN — ❌ ZERO scale-downs** (poll's stale view kept the overload label ≥5% through demand_drop → churn guard suppressed until the last second; verified via ground-truth `resource_stats` phase column + `client_requests`: demand identical ~1680 reqs in both arms, difference is the **controller's view**, not the workload — Arm A had already scaled down so its windows looked idle, Arm C's full fleet kept reporting ~20 reqs/window at ~10% CPU). **Fix = `idle_tail` phase added to the canonical `phases_rq1_stress_plateau.json`** (420 s, rate 0.05, client_fraction 0.05 after demand_drop) so the lossy arm observes a clean-idle window, the guard releases, and scale-down can arm/fire → C7 measurable for every arm. **Arm C re-run `rq1_g2_armC_rerun` (`20260805_184345`, exit 0) — ✅ PASS:** 7 real removals (lan1 storage dyn3/dyn5 + compute dyn2/dyn6/dyn4, lan2 storage dyn5/dyn1) all inside `idle_tail` (started 19:05:06Z, removals 19:08–19:13); loss penalty intact (delivered 55/165 ≈ 0.33, ~81/83 missed overload); demand_drop clean (p50 0.007 s). `idle_tail` applies to all arms (fair tail, +7 min/run). **Arm D `rq1_g2_armD_dryrun` (`20260805_193236`, exit 0) — ✅ PASS (gate f + g):** delivered fraction 0.3333 both LANs, delivery-delay p50 0.438 s (p95 0.83, max 0.92 — sub-second fresh, completing the 2×2 A fresh+complete · B stale+complete · C stale+lossy · D fresh+lossy), 8 real removals in `idle_tail` (lan1 storage dyn1 + compute dyn6/dyn2/dyn4; lan2 storage dyn3/dyn5/dyn1), 0 gap/err.
  **lan2 asymmetry diagnostic (gate h) — ✅ PASS 2026-08-05:** all four arms BALANCED (v1's Arm C lan2 plateau asymmetry does NOT reproduce under v2 open-loop). Plateau |lan2−lan1| failure/timeout pp: A −0.38/−1.48, B +0.15/+0.14, C −0.03/−3.09, D −1.29/−4.59 — all < 2 pp threshold; delivered windows equal per LAN (354/354, 347/347, 138/138, 144/144); offered ratio 1.00 all arms. Retained `lan2_asymmetry_preflight.csv` (n=1/arm — pre-flight only, replicate verdict pending the campaign). **ALL pre-flight gates PASS — 20-run campaign ready.**
