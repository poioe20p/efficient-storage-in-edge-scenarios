...
- **🛠️ G2 RETUNE (2026-08-04, root-caused collapse → Option A).** The true open-loop G2 (`20260804_165925_rq1_delivery_ep_calib2`) collapsed: lan1 timeout 87.7%, lan2 68.2%, dropped 3.1%/15.5%, lan1 dyn2 OOM-killed + base server restarted, lan1 telemetry silent from w54 → only 14 overload windows detected. **Root cause (not an independent lan1 bug):**
  ...
  **Changes (Option A — new per-RQ phase file, control-group file untouched):**
  ...
  - `_rq1_launch.sh` now uses `PHASES_CONFIG=testing/phases_override/phases_rq1_stress_plateau.json` (formalized as `source/scripts/testing/rq1_launch_run.sh`, which also carries the pool-12 value).
  **G2 calib4 (2026-08-04): pool 6 → 12.** With the data path fixed, detection was stable (121/121 overload, 100% delivery, no OOM) but service still collapsed at rate 3.0 (p50 24–34 s, timeout ~63%). Pool 6 sat at the DB-path knife's edge (~50 DB ops/s/LAN demand vs ~48 capacity; storage idle ~25%; telemetry proven lossless). Pool raised to 12 in all 4 env files + launcher (~96 DB ops/s, ~2× headroom, keeps rate 3.0).
  **G2 calib6 (2026-08-04): pool 12 NEGATIVE — workload re-anchored.** calib6 (pool 12 + absent-cleanup slot fix) still collapsed (p50 22.2/30.5 s, timeout 68%, completion ~29%, delivery 124/124). Root cause traced to a **control-loop churn collapse** (every DB spike coincided with a churn event; fleet churned 25 adds/17 removes; ~1 compute node for 76/125 windows), not the DB pool. **Driver-mismatch finding:** control/RQ2 “proven-stable” rates were measured under the legacy sync (closed-loop) driver; RQ1's open-loop driver was never rate-calibrated. **Fixes:** (1) `_HOUSEKEEPING_OVERLOAD_GATE` churn guard (default ON, all RQs) suppressing absent-cleanup + scale-down while overloaded; (2) open-loop rate sweep 2.0/2.5/3.0 (Arm A, seed 2001, pool 12 held) to re-anchor the plateau at a bounded-overload rate (failure ≤ 5–10%, stable fleet, overload detected). **Re-validation = the sweep**; no campaign block until a bounded rate is locked.
  **G2 gate run `rq1_g2_rate15_p6` (2026-08-05, rate 1.5 + pool 6 + churn guard + hysteresis, `20260805_065605_rq1_g2_rate15_p6`, exit 0) — ❌ FAILED.** Stable fleet (0 removals), no OOM, telemetry lossless (124/124 both LANs, 0 missed), dropped 0, overload detected (lan1 115/124, lan2 119/124), scale-up to dyn5/dyn6 — but plateau collapsed identically to pool 12: lan1 p50 16.2 s / timeout 68.3% / completion 31.0%; lan2 p50 16.8 s / timeout 66.4% / completion 32.6% (pool 12 rate-1.5: p50 15.6/16.9 s, timeout 62–67%, completion 33–37%). ⇒ **pool size DISPROVED as root cause.** Secondary: all `demand_drop` scale-down decisions were `absent` no-ops — no real removal executed. Open: RQ1 compute endpoints (`feed_ranking` 40% + `service_pressure` 30% at `EDGE_CPUS=0.15`) vs RQ2's pure-DB mix; scale-down no-op root cause. **No campaign block may start until the plateau is stabilized.**
  **G2 gate run `rq1_g2_rate12_mix_ec25` (2026-08-05, `20260805_074127_rq1_g2_rate12_mix_ec25`, exit 0) — ✅ PASS, plateau LOCKED.** Three-fix re-anchor (rate 1.5→1.2, mix rebalance, `EDGE_CPUS` 0.15→0.25) after the rate15_p6 FAIL confirmed the root cause as **compute-tier saturation**: edge CPU 55–73% med / peaks 99% at `EDGE_CPUS=0.15`; 70% CPU-heavy mix share. Result: lan1 p50 2.32 s / timeout 8.9% / completion 88.5%; lan2 p50 1.46 s / timeout 7.4% / completion 90.4%; delivery 124/124 (0 missed), dropped 0, no OOM, scale-up + **real scale-down removals** both fire. Remaining: lan1 overload detection 66/124 vs lan2 120/124 (asymmetry to track, not a blocker). **Plateau rate 1.2 + rebalanced mix + `EDGE_CPUS=0.25` LOCKED for the campaign.**
  **DB-op demand accounting (corrected 2026-08-05, reviewer-caught):** the per-request DB-op math used to justify the rate 1.2 margin was incomplete — `feed_ranking` does cost 3 ops/req (1 `user_profiles` + 1 `content_items.find` per LAN ×2) **and `content_lookup` costs 2 ops/req when a requester is sent** (`content_items.find_one` + `user_profiles.find_one`; the generator always sends `requester=<user_id>`). Correct plateau mix: 0.35·2 + 0.2·3 + 0.2·0 + 0.15·1 + 0.1·1 = **1.55 ops/req → ~45 DB ops/s/LAN** at rate 1.2 (not ~35). The empirical plateau passes (the collapse was compute-CPU saturation, and the 1.2 rate cleared it), but the “below RQ2's ~42” safety-margin rationale does NOT hold under consistent accounting — RQ1's real demand sits at/above the documented collapse-adjacent level, so the campaign must watch replicate-to-replicate plateau stability closely (lan1 detection already marginal at 53%).
  **Re-validation run pending** (Arm A `event_preserving`, open-loop, rate 3.0 + pool 12). **No campaign blocks may start until it passes** (bounded overload: no OOM, no telemetry silence, dropped ≤ 1% or window raised per G2 rule, scale-up + scale-down fire, lan1≈lan2 detection).
  **Per-arm scale-down arming (gate f) — A/B/C/D ✅ PASS, 2026-08-05.** Arm A `rq1_g2_rate12_mix_ec25` (4 real removals); Arm B `rq1_g2_armB_arm` (real storage removals in recovery_gap; plateau p50 1.60/1.58 s, timeout 2.9/3.1%, completion 93.5/93.3%); **Arm C `rq1_g2_armC_arm` FIRST RUN — ❌ ZERO scale-downs** (poll's stale view kept the overload label ≥5% through demand_drop → churn guard suppressed until the last second; verified via ground-truth `resource_stats` phase column + `client_requests`: demand identical ~1680 reqs in both arms, difference is the **controller's view**, not the workload — Arm A had already scaled down so its windows looked idle, Arm C's full fleet kept reporting ~20 reqs/window at ~10% CPU). **Fix = `idle_tail` phase added to the canonical `phases_rq1_stress_plateau.json`** (420 s, rate 0.05, client_fraction 0.05 after demand_drop) so the lossy arm observes a clean-idle window, the guard releases, and scale-down can arm/fire → C7 measurable for every arm. **Arm C re-run `rq1_g2_armC_rerun` (`20260805_184345`, exit 0) — ✅ PASS:** 7 real removals (lan1 storage dyn3/dyn5 + compute dyn2/dyn6/dyn4, lan2 storage dyn5/dyn1) all inside `idle_tail` (started 19:05:06Z, removals 19:08–19:13); loss penalty intact (delivered 55/165 ≈ 0.33, missed 81/83 overload — consistent with uniform 1/3 sampling over a 119–125/165-overloaded universe); plateau p50 5.59/2.58 s, timeout 16.0/12.9%; demand_drop clean (p50 0.007 s). `idle_tail` applies to all arms (fair tail, +7 min/run). **Arm D `rq1_g2_armD_dryrun` (`20260805_193236`, exit 0) — ✅ PASS (gate f + g):** delivered fraction 0.3333 both LANs, delivery-delay p50 0.438 s (p95 0.83, max 0.92 — sub-second fresh, completing the 2×2 A fresh+complete · B stale+complete · C stale+lossy · D fresh+lossy), **7 real removals in `idle_tail`** (lan1 storage dyn1 + compute dyn6/dyn2/dyn4; lan2 storage dyn3/dyn5/dyn1), 0 gap/err; plateau p50 6.82/5.58 s, timeout 19.4/14.8%.
  **lan2 asymmetry diagnostic (gate h) — ⚠️ CAVEATED 2026-08-05:** all four arms returned BALANCED **at n=1** (the diagnostic's n=1 path returns BALANCED by construction before any threshold check — a structural no-op, so this is NOT a replicate verdict). Failure deltas |lan2−lan1|: A −0.38, B +0.15, C −0.03, D −1.29 pp (all < 2 pp); **timeout deltas: A −1.48, B +0.14, C −3.09, D −4.59 pp — C and D EXCEED the 2 pp threshold** (lan2 lower). The delivered-window counts in the CSV (354/347/138/144) are raw delivery-log rows spanning setup/drain windows, not the universe-bounded per-run delivered counts (A 124/124, C/D 55/165) — the equal-per-LAN symmetry is real but the absolute values are not the analyzer's per-run counts. v1's Arm C lan2 asymmetry does not clearly reproduce, but this gate is evidence-limited at n=1; the campaign replicates are the real test. Retained `lan2_asymmetry_preflight.csv`. **Campaign-blocking gates PASS; gate (i) legacy sync-mode regression still pending (non-blocking for the open-loop campaign).**
  **`idle_tail` semantic caveat (2026-08-05):** adding the idle tail means C7 (scale-down ≥1 decision/LAN) and time-to-recover for the lossy arms are now measured against a phase added to force churn-guard release, not the pre-registered design. The guard (`_HOUSEKEEPING_OVERLOAD_GATE`, hysteresis 5) is itself a mid-G2 platform-safety addition. Within-factorial fairness is preserved (all arms share the file, +7 min/run), and C/D scale-down timing now partly reflects guard-release mechanics — must be stated explicitly in the results narrative (scale-down timing is a guard-interaction claim, not a pure delivery-semantics claim).
