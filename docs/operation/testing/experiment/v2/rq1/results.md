...
**Status**: 🔄 **IN PROGRESS — 6/20 runs done** (2026-08-05 21:47 → 2026-08-06 01:17); paused once at 3 flags → stability rule amended to collapse screening (2026-08-06); resuming at run 7 · Plan: [`rq1_v2_rework_plan.md`](rq1_v2_rework_plan.md) · Matrix: [`run_matrix.md`](run_matrix.md) §9
...
| Blocks 1–5 — 20 runs | — | 🔄 **6/20 done** (runs 1–6: Block 1 + Block 2 pos 1–2); resumed at run 7 (`ep_2`) | Delivery semantics clean on all 6 runs — A/B 0 missed overload (164/164, 163/163, 164/164); C/D delivered ≈ 1/3 (design, `POLL_INTERVAL_S=30` verified); scale-up fired, no OOM | Runs 1/5/6 hot under the strict 80/15 rule → pre-registered pause fired → **2026-08-06 amendment** (record in `run_matrix.md` §9.2): rule re-scoped to **collapse screening** (drop iff completion <60% / timeout >40% / OOM / A/B telemetry silence / C/D fraction ≠ 0.333±0.1); 80/15 retained as descriptive `plateau_temperature`; **lan1 asymmetry promoted to post-campaign root-cause** | Stability-rule amendment (docs only; no config change — all 6 runs valid) | Per-run checkpoints + `rq1_delivery_per_run.py`; stats at n=5 via `rq1v2_p3_01_stats.py`; lan2 asymmetry diagnostic at n=5 |
