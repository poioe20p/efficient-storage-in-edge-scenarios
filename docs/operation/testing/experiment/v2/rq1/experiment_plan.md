...
| 2026-08-07 | **v2 campaign COMPLETE (20/20 valid) + full analysis.** 4 arms × 5 runs (seeds 2001–2005) on the fixed platform; two replicates dropped + re-run for lan2 `plateau_unstable` (`ls_4`, `sp_5`); per-run gates + `rq1_delivery_per_run.py` all green; comparison graphs (n=5/arm), `rq1v2_p3_01_stats.py` (n=5), `rq1v2_p4_01_lan2_asymmetry.py` (n=5) run on `cloud-vm`; per-run `run_summary.md` ×20; results in `results.md` §v2 Final | Close RQ1 v2 as final thesis evidence: capacity ordering A (32.0 s) < B (57.5) < C (81.5) ≈ D (85.1) with d=−1.0 on both loss edges + fresh delay edge; info-age A < D < C < B; C8 non-surge NULL (all ≈ 0); C7 letter-pass with guard-conditioned real removals for C/D (idle_tail); decision-log real-removal gap (G8) confirmed at n=5; lan2 asymmetry migrated C→A (+2.64 pp plateau failure on A) — see `results.md` §v2 Campaign (criterion table + findings) |
| 2026-08-07 | **Post-campaign diagnostics + v3 platform-fix plan** — [`rq1_v3_platform_fix_plan.md`](rq1_v3_platform_fix_plan.md): all plateau timeouts/failures live in the no-backend (`unknown`) population (100% of both); named backends serve at 0% failure at any fleet size ⇒ scaling shows no user-visible benefit because the user bottleneck is the VIP→edge proxy path (conntrack disproved); lan2 asymmetry is the same population (4–45× worse on lan2); `resource_stats.server_count` undercounts the fleet; four fixable issues identified (routing-layer failures, lan2, G8 scale-down logging, designed-null plateau) with a fix plan. **Hold point — no fixes or runs started** | Record the investigations before any v3 re-run; RQ2 proves the scaling→user-quality chain is observable after re-calibration; RQ3's consequence is null (same shape as RQ1) |
