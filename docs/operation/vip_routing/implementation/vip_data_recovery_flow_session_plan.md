# VIP_DATA Recovery Epoch Flow and Session Model

**Status:** Implemented
**Primary file:** [app.py](../../../../source/docker/edge_server/source/app.py)
...
`app.py` seeds `_epoch_states` during module initialization, before telemetry
startup and before the housekeeping thread begins.
...
`timed_db(lan)` is now the boundary that leases the current epoch for the
request.

Request flow:

1. Resolve the LAN against `_epoch_states`.
2. Check the per-LAN circuit breaker.
3. Lease the current epoch and record `g.db_epoch_context` immediately.
4. Lazily create that epoch's `MongoClient` if needed.
5. Yield the database handle.
6. Release the epoch lease in `finally`.
...
This design intentionally does **not** claim backend exclusion. A fresh normal
or recovery epoch creates a fresh local connection attempt using its bound VIP,
but the controller may still choose the same backend again if its current
