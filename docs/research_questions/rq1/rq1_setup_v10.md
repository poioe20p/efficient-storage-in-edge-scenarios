# RQ1 v10 — Experiment Setup Declaration

> **Canonical reference** for how RQ1 v10 was tested. Values extracted from
> `experiment_plan_v10.md`, `phases.json`, `current_state_integrated.env`,
> `osken-controller.env`, and `scaling_config.py`.
> **Corresponding RQ doc**: [`rq1_v3.md`](rq1_v3.md)
...
Identical to v8's phase structure (the 9-phase cleanup-gap workload is the
...
| Parameter | v10 Value | v8 Value | Rationale |
|-----------|-----------|----------|-----------|
| `CLIENTS` | 96 (48/LAN) | 96 | Same as v8 |
| `MAX_DYNAMIC_COMPUTE` | 12 | 12 | Same as v8 |
| `MAX_DYNAMIC_STORAGE` | 8 | 8 | Same as v8 |
| `STORAGE_CPUS` | 0.08 | 0.08 | Same as v8 |
| `STORAGE_MEMORY` | 512m | 512m | Same as v8 |
| `EDGE_CPUS` | **0.15** | 0.30 | **Halved** — reduces static-node absorption capacity so the blind spot cascades into user-visible failure (see §2.1) |
| `EDGE_MEMORY` | 256m | 256m | Build script default — not passed explicitly |
| `CURL_MAX_TIME` | 30 s | 30 s | Same as v8 |
| `WAN_RTT_MS` | 185 ms | 185 ms | Same as v8 |

### 2.1 Why EDGE_CPUS = 0.15
...
throughput gap was noisy (−19% mean, σ=14K). One Poll-30s replicate matched
Push throughput.
...
The C2 pilot (2026-07-25) confirmed this: Push completed 89,028 requests at
2.0% timeout; Poll-30s completed 62,299 at 4.5% timeout — a 30% throughput
gap and 2.3× timeout increase with clean G8 passes on both runs.
...
Identical to v8 — no threshold or cooldown changes.

| Parameter | v10 Value | Default | Rationale |
|-----------|----------|---------|-----------|
...
| Delivery modes | Push (ZMQ), Poll-5s, Poll-12s, Poll-30s (HTTP) | Independent variable — full dose-response curve |
...
Same images as v8 — no rebuild needed.
...
4 modes × 3 replicates = 12 runs. **Sequential gate**: Push and Poll-30s
run first (6 runs). If the C2 separation replicates, Poll-5s and Poll-12s
complete the dose-response curve. If it fails, the campaign stops.

| # | Label | Mode | EDGE_CPUS | TELEMETRY_SOURCE | POLL_INTERVAL_S |
|---|-------|------|-----------|------------------|-----------------|
| P1–P3 | `rq1_v10_push_{1,2,3}` | Push | 0.15 | *(default — ZMQ)* | — |
| T1–T3 | `rq1_v10_poll30_{1,2,3}` | Poll-30s | 0.15 | `poll` | 30 |
| F1–F3 | `rq1_v10_poll5_{1,2,3}` | Poll-5s | 0.15 | `poll` | 5 |
| W1–W3 | `rq1_v10_poll12_{1,2,3}` | Poll-12s | 0.15 | `poll` | 12 |

All other parameters (`CLIENTS=96`, `WAN_RTT_MS=185`, `STORAGE_CPUS=0.08`,
`CURL_MAX_TIME=30`, `RANDOM_SEED=42`, `DATA_SEED=42`, `PHASES_CONFIG=testing/phases.json`,
...
See [`rq1_v3.md` §5](rq1_v3.md#5-how-it-is-measured) for full metric definitions.
