...
| Lee et al., Shard Manager (2021)                                 | The orchestrator monitors health/load, changes assignments, disseminates a new map through service discovery, and uses an ordered prepare/add/disseminate/drop sequence so graceful migration drops no requests.                                                        | Strong basis for lifecycle-safe dynamic membership and for treating dissemination to request routing as one ordered step in making capacity usable. | Its unit is shard-role migration, not compute-backend readiness.                                                                                                                     |
...
5. Use Llorens-Carrodeguas, Shard Manager, Kurma, and SmartLB as the main conceptual chain; use Belgaum/Neghabi/Abdelltif as delimiting or supporting sources.
