# RQ3 — Related Work: Evaluation Practice in Readiness Propagation & Elasticity
...
> **Bibliography note:** `tese/references.bib` currently holds 9 entries; the
...
The readiness-propagation blind spot, as established in `rq3.md` §3 — the
*same gap, three names*: monitoring ("visibility gaps", Yaseen), service
discovery ("freshness still a problem", Pourghebleh; no discovery-timing
category, Achir), and load-balancing/scaling ("synchronization-before-inclusion
delay", Wang et al.; the symptom in Pierro & Ullah). No study isolates the
...
`tese/references.bib` has **9 entries**; the RQ3 basis papers (`rq3.md` §6)
...
| An SDN load-balancer paper (Wang et al. SDNFV — to add formally) | the core seed ("notifies LB-C to include the instance") | B |
...
- `tese/literature_review/global_literature_review.md` — §2.1 (spawn-to-LB
  inclusion), §2.6 (SD blind spot), §3.1 (Pierro & Ullah symptom), §5.6
  (same gap, three names).
