...
| §3.2 Architecture (`sec:architecture_overview`) | Keep (two geo-distributed networks + WAN, OVS, double-VIP, 3-thread controller, telemetry pipeline). **Scope note (2026-08-16):** the WAN sits only on the inter-LAN data link and the evaluated demand never traverses it — the control plane (aggregator→controller, controller↔controller) is host-local. Present the two-site topology as *cross-site-aware control plane + single-site data path*. |
