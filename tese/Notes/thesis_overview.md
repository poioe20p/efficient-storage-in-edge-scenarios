...
The review opens with the platform technologies on which the testbed rests, each introduced for what it is and why it suits the edge scenario. Edge computing brings resources closer to users, trading per-site capacity for latency; the reviewed platform landscape ranges from production public edge platforms (Xu et al., Alibaba NEP) to serverless and joint function-and-state placement designs (LAVEA; Pelle et al.), taxonomised by Liu et al. SDN separates the control plane from forwarding so routing can be reprogrammed centrally, the capability the double-VIP admission model exploits (Baktir et al.; Liatifis et al.). Containers provide lightweight, near-native isolation for constrained edge nodes (Kaiser et al.), at the cost of startup latency dominated by image download on limited bandwidth (Lou et al.), while edge orchestration tools remain only partially adapted to dynamic edge environments (Cilic et al.). Document-oriented NoSQL stores fit heterogeneous edge data (Eyada et al.), are surveyed for edge and fog settings by Ferreira et al., carry measured resource-utilization trade-offs (Mansouri et al.), and support replication models on which geo-distributed deployments draw (Mealha et al.). This section is background: it justifies the technology choices without contributing evidence to any research-question gap.
