...
### 3.1 Platform Technology Background

The review opens with the platform technologies on which the testbed rests, each introduced for what it is and why it suits the edge scenario. Edge computing brings resources closer to users, trading per-site capacity for latency; the reviewed platform landscape is taxonomised by Liu et al., resource optimisation in edge and SDN-based edge computing remains an open challenge (Nain et al.), and designs range from serverless to joint function-and-state placement (LAVEA; Pelle et al.). SDN separates the control plane from forwarding so routing can be reprogrammed centrally, the capability the double-VIP admission model exploits (Baktir et al.; Waseem et al.). Containers provide lightweight, near-native isolation for constrained edge nodes (Kaiser et al.), with cross-layer orchestration proposed for containerised applications (Sofia et al.), while edge orchestration tools remain only partially adapted to dynamic edge environments (Cilic et al.). Document-oriented NoSQL stores fit heterogeneous edge data (Eyada et al.), are surveyed for edge and fog settings by Ferreira et al., carry measured resource-utilization trade-offs (Mansouri et al.), and support replication models on which geo-distributed deployments draw (Mealha et al.). This section is background: it justifies the technology choices without contributing evidence to any research-question gap.

### 3.2 Monitoring and Telemetry

Monitoring research establishes that delivery design matters. The state of the art rests on a common pipeline — per-node collectors, an aggregation layer, and pull-based consumption from a time-series store, as in OSM-based NFV and containerised edge stacks — and the thesis derives from this pipeline the three delivery regimes it varies: prompt and complete, delayed event-preserving, and latest-state. Within this pipeline, Huang and Pierre's AdapPF varies Prometheus scrape intervals in geo-distributed cluster federations and shows that coarse collection can reduce scheduling quality, while adaptive collection can reduce monitoring traffic, with aggregation proposed as a way to make monitoring scale. Yaseen's survey of programmable network-wide monitoring identifies pull-based visibility gaps and calls for monitoring to be integrated with control and automation, a co-location a production 5G SDN controller demonstrates without scaling. Usman et al.'s observability survey for edge and container-based microservices leaves the definition of appropriate measurement intervals as an open problem, naming the dimension RQ1 varies.
