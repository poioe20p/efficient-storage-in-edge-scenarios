...
operate.
    ...
    Across these areas, the review follows the three interfaces of
...
Before the field areas, \ref{sec:platform_background} surveys the platform
technologies on which the testbed rests: edge platforms, SDN and OpenFlow,
containerisation, and NoSQL document stores, each introduced for what it
is and why it suits the edge scenario. The review methodology is described
in \ref{sec:review_method},
...
solution. Four technologies define the testbed evaluated in this
dissertation: edge computing, which sets the deployment scenario; SDN
with OpenFlow, which provides the programmable network control;
containerisation, which supplies the unit of compute provisioning; and
NoSQL document stores, which hold the service state.

    For each technology, the section states
what it is and why it matters in edge deployments,
and reviews the relevant studies that establish its role.
How the technologies are applied is the subject of Chapters
\ref{ch:system_architecture} and \ref{ch:implementation}.

\subsection{Edge Computing and Edge Platforms}
Edge computing brings computational resources closer to end users, by
trading per-site capacity for lower latency and for relief of the
backbone, as previously presented in \ref{sec:context_motivation}.

    The platforms that materialise this paradigm have been taxonomised by
Liu et al., who observe that the different forms of edge computing
emerge from different design demands: cloud providers push services and
computation to the edge to exploit locality and improve user
experience, IoT applications pull computation from the cloud towards
the edge to handle the large volume of data generated by the devices
themselves, and hybrid systems combine the two, using edge resources to
filter and preprocess data while the cloud runs the heavier analytics
\parencite{Liu2019SurveyEdgeComputingSystemsTools}. 
    
    Together, these three forms illustrate part of the importance of
edge computing. Resource optimisation in such environments remains an open
challenge: Nain et al. systematically review resource optimisation
in edge and SDN-based edge computing and the methodologies proposed
for managing those resources, and identify the integration of SDN with edge
computing as a continuing challenge
\parencite{Nain2024ResourceOptimizationEdgeSDNEdge}. 
    
    At the other end of the design space, serverless edge platforms
such as LAVEA, which targets latency-aware video analytics at the edge,
monitor execution continuously and use the monitoring information for
task placement \parencite{Yi2017LAVEA} in order to lower latency for the
video analytics. Pelle et al. jointly optimise function and
state placement with runtime monitoring and redirection in an integrated
platform for latency-critical applications such as assisted driving
\parencite{Pelle2022CostLatencyEdgePlatform}. 

    The reviewed platforms thus span provider-managed infrastructure
and monitoring-informed placement, combining monitoring, placement,
and redirection in different ways, and neither design demonstrates
the runtime scaling of the service itself; in none of them are the
interfaces between the functions they do combine exposed as independently
tunable dimensions, the point developed in \ref{sec:lit_three_layer_separation}.
...
Orchestration tools must also adapt to the
edge, and Cilic et al. evaluate Kubernetes and its edge-oriented
distributions, K3s, KubeEdge, and ioFog, in real edge execution
environments, and find that none of the
selected tools fully covers monitoring and scheduling for dynamic edge
environments \parencite{Cilic2023PerformanceEvaluationContainerOrchestrationTools}.
...
documents within collections, provides a flexible schema, and offers
native replica sets, through which members can be added to and removed
from a deployment while it keeps serving.
...
\end{figure}
    ...
    Eyada et al. compare MongoDB 
against MySQL for IoT data management and find that MongoDB outperforms
the relational system in latency and in the database size that remains
practical as the amount of tested data grows, while requiring fewer
resources \parencite{Eyada2020PerformanceEvaluationIoTDataManagement}.
...
capacity action that RQ2 implements and evaluates.
...
a single orchestration stack for industrial IoT
...
the latest state. How the different forms such as event-preserving delivery,
delayed event-preserving delivery, and latest-state delivery affect
resource orchestration is part of the subject of study in this thesis.
...
characterisation, and does not run over a stateful service. 
    ...
    The same line of work treats monitoring overhead as a recognised bottleneck in
geo-distributed deployments, proposing aggregation so that monitoring can
scale with the federation \parencite{Huang2024AggregateMonitoringGeoDistributedKubernetes},
...
work echoes the same open dimension: Usman et al., surveying
...
response, and transient service quality within these mentioned constraints.
...
that a single controller re-programs the forwarding paths while new
instances are brought up; the monitoring and lifecycle components that
feed and execute the scaling decision remain external, and every action
provisions the same kind of resource, a new function instance
\parencite{Llorens2021SDNBasedHorizontalAutoScalingLoadBalancing}.
...
process \parencite{Toka2021MachineLearningScalingManagementKubernetes}. The
work improves the decisions taken inside the pipeline, but it
provisions only compute replicas, with no storage action and no
classification of which tier is the bottleneck.

     Ju et al. scale edge applications on Kubernetes with a proactive pod
autoscaler that forecasts workload ahead of demand and adjusts
CPU-intensive application replicas accordingly
...
the provisioned unit remains a compute replica, and none considers a
storage action.
...
violations, so the tier that determines service quality is exactly the
one the scaling loop does not manage
\parencite{Nikravesh2017ImpactDatabaseLayerAutoScaling}. Dogani et
al. record the same boundary across their surveyed corpus: adding
resources does not necessarily raise throughput when the bottleneck
lies elsewhere
\parencite{Dogani2023AutoScalingTechniquesContainerCloud}. Within that
corpus, the only awareness of the effect is the Waterfall auto-scaler,
a machine-learning system that accounts for how scaling one
microservice can shift the bottleneck towards downstream services; the
awareness is cross-service and compute-only, and no surveyed system
selects a storage action.
...
but the bottleneck it locates is always a microservice and the action
it takes is always a compute-replica count, so the selection between
...
forecast of future workload built over historical access patterns
...
management efficiency. 
    ...
    Pelle et al. keep function and state placement in sync within
a single optimisation loop over an integrated edge platform
...
The field's own framing carries the default: Qu et al. describe the web
application as split into application logic and a database tier, and
...
\begin{table}[htbp]
    \centering
    \caption{Evidence on capacity-action selection in the reviewed corpus.}
...
\end{table}
    ...
    Within the reviewed corpus, therefore, auto-scaling studies when to
...
reviewed corpus treats them as a joint optimisation of the mix rather
...
\ref{sec:platform_storage}. RQ2 isolates exactly that choice: under
fixed telemetry delivery and readiness admission, the thesis compares
fixed-priority and bottleneck-aware selection of the capacity action
under compute-bound and data-access-bound demand, and measures
service recovery and resource management efficiency when both
...
% Add (approved 2026-08-22): delimit Nguyen et al. 2021 5G UPF (co-located AS+LB, nothing
% tunable) and Baktir (periodic discovery + registration described, never timed or varied).

\section{Resource Orchestration on SDN}\label{sec:lit_sdn_orchestration}
Resource orchestration coordinates the compute, network, and storage
resources of an infrastructure so that the services running on it
receive what they need. In SDN-based edge platforms this coordination
spans the SDN controller, the container runtime, and the data store,
and the challenge is to keep these layers working together as demand
moves across sites. This section reviews the orchestration context and
the boundaries of the surveyed work.
