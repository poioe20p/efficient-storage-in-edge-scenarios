...
\begin{itemize}
    ...
    how does bottleneck-aware scale-out, compared with fixed single-tier
...
\item \textbf{Objectives of a solution.} As defined in Section~\ref{sec:objectives},
...
In a stateful edge service, however, the data tier is
...
allocation of compute and data resources, Section~\ref{sec:monitoring_decision}
...
data tier can be addressed without conflating the two. The two dimensions are
described in turn.
...
readiness-admission mechanism is the interface examined for the third
research question.
...
idle condition defined in Section~\ref{sec:scaling_policy} to the data tier's
...
resource tier it concerns (service or data) and the action requested (scale-up
