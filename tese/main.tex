...
% Review before sending to professor because we seem to have more thatn 39 papers
...
examines the shared default that emerges.
    ...
    This thesis responds to that default with an experimental
arrangement: two peer SDN
controllers host controller-side telemetry consumption, scaling
decision-making, and routing within their respective control processes.
Aggregated telemetry arrives through an explicit, configurable delivery
interface, while policy and lifecycle hooks expose capacity-action
selection and readiness admission as independently variable dimensions.
The corresponding timing segments can therefore be measured in
isolation, as presented in \ref{ch:system_architecture}.
...
whether virtual machines or containers, and treats auto-scaling as a
runtime-adaptation requirement shared by the analysed application types
\parencite{Taherizadeh2017AutoScalingApplicationsEdgeComputing}.

    When SDN enters the loop, the traffic side is added but the decision
space remains single-option: Llorens-Carrodeguas et al. couple SDN-based
traffic steering with horizontal scaling of virtualised network functions,
so that a single controller re-programs the forwarding paths while
instances of the same pre-dimensioned function pool are activated or
deactivated. Metric collection and the lifecycle operations that feed and
execute the decision remain external, and the decision itself reduces to
whether and how many instances of that one function to activate, leaving
no selection among alternative capacity actions in response to the
observed bottleneck
\parencite{Llorens2021SDNBasedHorizontalAutoScalingLoadBalancing}.
...
systems the question answered is how many replicas of the one compute
tier to run and where, while the possibility of scaling a different kind
of capacity does not arise.
...
traffic in an elastic SDN service.
    ...
    Production systems mechanise the same interface without measuring it.
...
container-readiness admission, and its evaluation neither varies the
propagation semantics of the transition nor reports the time between
readiness and usable capacity
...
capacity is therefore a recognised cost in production routing, yet
neither system isolates how readiness state propagates to the routing
plane or compares direct lifecycle notification with periodic discovery
of the same state.
...
concern. Its evaluation, however, does not isolate the interval from
verified application readiness to routing eligibility, nor compare direct
lifecycle notification with periodic discovery of the same readiness
state.

    Across this literature, therefore, the selection of a backend among
eligible candidates is well characterised, while the transition that makes
a newly provisioned backend eligible remains unmeasured: no reviewed work
isolates the interval between verified readiness and routing eligibility,
nor compares direct lifecycle notification with periodic discovery of the
same readiness state. This dissertation isolates that transition, holding
the application-readiness criterion and the selection function constant,
...
surveyed work.
    ...
    The surveyed stacks bind these layers with different degrees of
coupling. Okwuibe et al. assemble the stack closest to the one built
in this dissertation: an SDN controller, a Kubernetes cluster, and a
...
Wang et al. survey SDN-enhanced edge computing from the network
layer upwards and document architectures in which SDN and NFV
coordinate placement, traffic, and caching, showing that SDN's role
at the edge extends beyond forwarding into orchestration
\parencite{Wang2019SoftwareDefinedNetworkingEnhancedEdge}.
...
% \end{figure}
    ...
    Within the reviewed corpus, orchestration platforms either
coordinate the three functions by construction or leave their
interfaces as boundaries they do not vary; in neither mode is an
interface varied as an independent variable.
...
as a standalone problem, with evaluation practice fragmented across
the field \parencite{TabatabaeeMalazi2022DynamicServicePlacementMultiAccess}.
...
\parencite{Saurez2021OneEdge,Tamiru2021Mck8sOrchestrationPlatformGeoDistributed,Bogdanov2018Kurma}.
    ...
    Within the reviewed corpus, therefore, the separation is documented
...
Monitoring and telemetry studies the collection and delivery of
demand evidence, yet the delivery mechanism itself remains a given:
no reviewed work distinguishes delayed event-preserving delivery from
latest-state delivery, and none traces either through a stateful
service's scaling and admission path. Auto-scaling studies when to
scale and how many resources to add, while the kind of capacity the
action provisions remains an architectural given, the database tier
being, in the field's own words, dynamically unscalable and ignored
by auto-scalers
\parencite{Qu2018AutoScalingWebApplicationsClouds}.
Load balancing on SDN studies the selection of a backend among
eligible candidates, while the transition that makes a newly
provisioned backend eligible has been neither isolated nor compared
between direct lifecycle notification and periodic discovery. And the
orchestration platforms that do coordinate these functions do so by
construction, none varying an interface as an independent variable.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.34\textwidth} p{0.58\textwidth}}
        ...
        Under stateful workloads, does selecting compute or storage
        scale-out from the observed bottleneck improve recovery and
        resource efficiency? \\
        Ready backend to traffic (RQ3) &
        How much does readiness admission, under direct lifecycle
        notification versus periodic discovery, delay a ready
        backend becoming usable capacity? \\
        \bottomrule
    \end{tabular}
\end{table}
...
call is loudest where two fields describe the same phenomenon
without citing each other: monitoring speaks of visibility gaps
between collection instants, and service discovery speaks of
registry freshness, two names for one blind spot that no reviewed
work measures
