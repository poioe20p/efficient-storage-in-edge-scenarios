...
NoSQL document stores, which hold the service state. For each
technology, the section states what it is and why it matters in edge
deployments, and reviews the relevant studies that establish its role.
...
\parencite{Liu2019SurveyEdgeComputingSystemsTools}. Together, these
three forms trace part the importance of edge computing to a single premise:
moving computation closer to the users and to the data they generate,
whether the initiative comes from cloud providers, from IoT
deployments, or from the combination of both.
Production public platforms illustrate the practical state of the art:
the public edge platform measured by Xu et al. separates
provider-controlled virtual machine placement from customer-controlled
request scheduling, and the resource imbalance that results shows that
coordination between placement and scheduling is a live operational
concern rather than a solved one
\parencite{Xu2021FirstLookPublicEdgePlatforms}. At the other end of the
design space, serverless edge platforms such as LAVEA monitor execution
continuously and use the monitoring information for task placement
\parencite{Yi2017LAVEA}, and Pelle et al. jointly optimise function and
state placement with runtime monitoring and redirection
\parencite{Pelle2022CostLatencyEdgePlatform}. The reviewed platforms
thus span provider-managed infrastructure and monitoring-informed
placement, combining monitoring, placement, routing, and scaling in
different ways, and in none of them are the interfaces between these
functions exposed as independently tunable dimensions, the point
developed in \ref{sec:lit_three_layer_separation}.
...
controller instead of being configured device by device. Baktir et al.
survey how edge computing benefits from this decoupling and organise the
gains into benefit areas such as centralised visibility and programmatic
traffic management, arguing that SDN can act as the enabler that lowers
the complexity barriers of edge deployments
\parencite{Baktir2017HowCanEdgeComputingBenefit}. The OpenFlow protocol
implements the decoupling through match-action flow rules installed on
the switches, and Liatifis et al. trace the evolution of the paradigm
from OpenFlow towards fully programmable data planes with P4
\parencite{Liatifis2023AdvancingSDNOpenFlowP4Survey}. In its OpenFlow form, a single
controller re-programs the forwarding behaviour of the network by
installing and updating match-action flow rules, so that network
behaviour can follow the life cycle of the services it carries without
reconfiguring the forwarding devices individually.
...
virtual machines, a property that suits resource-constrained edge nodes.
On ARM-based edge devices, Kaiser et al. benchmark Docker, Podman, and
Singularity and find that Docker achieves near-native start-up times,
...
Start-up is not free, however: Lou et al. show that container start-up
latency at the edge is dominated by image download over limited
bandwidth rather than by image installation, and their layer-aware
assignment reduces total start-up latency by forty to sixty percent
\parencite{Lou2023EfficientContainerAssignmentLayerSequencing}.
Beyond individual containers, orchestration tools must adapt to the
edge, and Cilic et al. evaluate Kubernetes and its edge-oriented
distributions, K3s, KubeEdge, and ioFog, and find that none of the
...
documents rather than relational tables, which suits the heterogeneous
data of edge and IoT services and scales horizontally through
replication and sharding. Eyada et al. compare MongoDB against MySQL for
IoT data management and find that MongoDB outperforms the relational
system in latency and in the database size that remains practical as the
amount of tested data grows, while requiring fewer resources
...
Mansouri et al. benchmark the resource utilisation of Cassandra,
MongoDB, Redis, and MySQL in edge-cloud deployments, including on
constrained devices such as Raspberry Pi, and show that the choice of
store changes energy, bandwidth, and storage consumption
\parencite{Mansouri2023ResourceUtilizationDistributedDatabasesEdge}.
Among these stores, MongoDB also provides native replica-set
replication, through which members are added to and removed from a set
while it keeps serving, so that a document store can itself be scaled
horizontally as demand shifts.
The replication models on which such geo-distributed deployments draw
are exemplified by Mealha et al., whose cloud/edge replication over the
...
omits scaling altogether. At the platform level, adjacent observability
...
\parencite{Usman2022SurveyObservabilityDistributedEdgeAmp}. These works sit at the network level rather
...
response, and transient service quality.
    ...
    The delivery mechanisms of the testbed realise these regimes
concretely. Telemetry leaves the edge servers over ZeroMQ message
queues, an asynchronous high-throughput messaging library
\parencite{Hintjens2013ZeroMQMessagingManyApplications}, and is also
available through HTTP polling, both sources converging on a single
aggregation point before the decision. The distinction between the
regimes therefore maps onto concrete transports: the event-preserving
source preserves every completed telemetry window in order, and the
latest-state source answers each poll with the newest sample only,
discarding intermediate observations.
...
% Add (approved 2026-08-22): Dogani et al. 2023 taxonomy (99_reference/autoscaling_background)
% with the Waterfall delimiter: bottleneck-shift awareness exists but is compute-only and
% cross-service; tier-action selection remains the thesis's novelty.
...
\section{Edge Storage Related Work}\label{sec:lit_edge_storage}
A final strand of related work concerns the data itself. Popularity-based
placement distributes content according to access frequency, but Wei and
Wang describe their placement decision as static and list online
placement as future work
\parencite{Wei2023PopularityDataPlacementLoadBalancing}.
Forecast-driven retention by Lujic et al. instead discards data predicted
to be unnecessary, the inverse of capacity scale-out
\parencite{Lujic2017EfficientEdgeStorageManagementNear}. Malazi et al.
survey dynamic service placement in multi-access edge computing and
record fragmented evaluation practice across the field
...
selecting a capacity action. The survey by Ferreira et al. frames the
design space of edge and fog databases
\parencite{MerujeFerreira2024DatabasesEdgeFogEnvironmentsSurvey}, and
the cloud/edge replication of Mealha et al. over the MongoDB data model
leaves \enquote{a dynamic placement strategy of either full or partial
databases} to future work \parencite{Mealha2019DataReplicationCloudEdge}.
None of these works selects storage scale-out from a live observed
bottleneck, which is the capacity action that RQ2 implements and
evaluates.
