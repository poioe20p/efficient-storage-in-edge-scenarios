...
sites.
    ...
    The thesis does not claim that this SDN-based apparatus is
...
% DSR paradigm — Hevner et al. (2004). Evaluation procedure (absorbed from the
% removed Evaluation Methodology chapter): per-run experimental unit; scheduled
% open-loop driver preserving a pre-specified offered-load process; pre-registered
% Mann-Whitney U + Cliff's delta on pre-registered edges, CIs reported in full;
% per-phase aggregation; validity (internal/external/repeatability).
...
% TODO: Brief paragraph outlining the 6 chapters.
