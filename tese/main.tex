...
independently reveals how these dimensions affect resource orchestration 
...
a deliberate design choice, the platform can change the
consumption and delivery of demand evidence, the scaling action
selected and the admission of ready capacity while keeping the others
fixed, and measure the effect of that single change.
...
does not use. Auto-scaling strategies are commonly grouped into
...
activate, and the data tier those functions depend on is held fixed by
the design. In a stateful edge service, however, that fixed tier is
...
systems the quantity that is forecast and adjusted is the number of
compute replicas. The improvement they pursue is a better forecast of
how many instances to run; which tier holds the limiting resource does
not enter their problem.
...
takes is always a compute-replica count, so the selection between
...
idle. In SDN networks this distribution is programmed centrally, where
the controller installs forwarding rules that steer each flow to a
...
\subsection{Admission as a Lifecycle Interface}
...
studies is the interval between a backend becoming ready and traffic
actually reaching it. This dissertation isolates that interval. It varies
readiness-admission coordination while holding the application-readiness
criterion and selection function constant, and measures the time until
newly provisioned capacity becomes selectable and serves traffic.
Direct lifecycle notification and periodic discovery are
...
in places, combined, but no reviewed work varies one interface while
holding the others fixed and traces the effect through a stateful
service. The closest reviewed precedent is AdapPF, which varies
...
constant, and the demand-to-capacity chain is treated as a sequence
of separately measured interfaces, whose aggregate cost this
dissertation deliberately does not claim.
