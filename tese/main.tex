...
% The problem narrows the broader goal stated in the dissertation proposal,
% more efficient resource management at the network edge, into a specific
% and previously unexamined slice: the coordination gap between
% independently operating control loops.

%   This thesis investigates them directly. The work originates in the
% broader goal, set out in the dissertation proposal, of more efficient
% resource management at the network edge; the review that follows narrows
% that goal to the coordination gap as the specific, previously unexamined
% slice through which orchestration efficiency is eroded.
...
real-world problems, the research contribution lies on the knowledge generated
...
\begin{enumerate}
    ...
    \item An experimental characterisation of readiness propagation (direct
    ...
    event schema, identifying and highliting each interface's impact across
    diferent scenarios under various conditions.
\end{enumerate}
...
This chapter presents the background and related work that motivated the research,
contextualised for resource orchestration in edge platforms.
It is organised around the three interfaces of the
demand-to-capacity chain studied in this thesis: monitoring and telemetry,
which feed the scaling decision; auto-scaling, which selects the capacity
action; and load balancing on SDN, which distributes traffic across the
available backends and determines when newly provisioned capacity joins
that distribution.
A fourth section covers resource orchestration on SDN, the platform context
in which these functions operate. Because these functions typically run as
independent modules with their own control loops, each section closes by
examining what the reviewed corpus says about the handoff delays between
them. The review methodology is described in \ref{sec:review_method}, and
the chapter concludes with the research gaps derived from the corpus in
