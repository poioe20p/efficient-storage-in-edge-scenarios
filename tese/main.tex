...
can suffer from coordination delays that arrise from the independent control loops through
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{images/global-internet-traffic-over-the-years.pdf}
...
multimedia, highliting the need for fast and capable data processing. These are just
...
pressure on the core networ, since data can be processed and stored locally,
...
The core objective of this thesis is to investigate and characterise the
interfaces between telemetry delivery, capacity to action selection, and traffic
admission in a stateful edge service operating under varying demand shifts.
The investigation rests on the idea that co-locating these functions
in a single control process suppresses the coordination gap, that is,
the handoff delays that accumulate when monitoring, scaling, and routing
run under separate control loops, and thereby exposes each interface to direct measurement.

    With no cross-component handoffs, the delay contributed by an interface can be
attributed to that interface alone. This is the basis of the experimental design,
not a claim that co-location is superior. Because the co-located components are coupled,
making each interface independently variable is itself a deliberate design of the
platform, which lets each interface be varied in isolation while the others are held
constant, so that its impact can be measured not only on the time from demand to usable
capacity but also on service quality, on how load is distributed across resources, and
on how flexibly the service responds to demand variation.
...
\end{figure}
    ...
    Design science produces and evaluates purposeful artefacts that address a class
of real-world problems, the research contribution lies in the knowledge generated
...
operating control loops in edge orchestration which is justified by the state of the art reviewed
...
\ref{ch:literature_review} establishes both that the layers are typically
...
\item \textbf{Objectives of a solution.} The objectives that a solution should
accomplish are defined in \ref{sec:objectives}: to investigate and characterise
the specific orchestration aspects among the control-loop interfaces, more specfically 
telemetry delivery, scaling action and selection, and readiness propagation to traffic 
admission, through a co-located basis that suppresses the coordination gap and in which each
interface is deliberately engineered as an independently variable dimension.
...
their usefulleness for the platform: SDN provides a single programmable control
...
\item \textbf{Demonstration.} The artefact was deployed and executed as 
an emulated edge environment with two simulated sites with
scheduled workload phases reproducing demand shifts in order to
demonstrate that the platform operates end-to-end from demand
...
pre-measurement and statistical procedure where each experimental is composed of
multiple independent runs each developed as an experimental unit, a scheduled open-loop
driver preserving the offered load, and primary comparisons which is detailed in
\ref{sec:experimental_setup}. Internal, external and repeatability validity threats
...
The main contribution of this dissertation is the experimental characterisation
of the three interfaces of the demand-to-usable-capacity chain in a stateful
edge service, made measurable by a co-located platform that suppresses the
coordination gap and varies each interface independently. This contribution
rests on the following five elements:

\begin{enumerate}
\item A co-located SDN-based experimental architecture, in which monitoring,
scaling and routing run in a single controller and each of the three
interfaces is a deliberately configurable, independently variable dimension,
turning previously uncharacterised interface questions into measurable ones.

\item The characterisation of telemetry delivery semantics (RQ1): how verified
event-preserving, delayed event-preserving and latest-state delivery affect
overload observability, scaling response and transient service quality.

\item The characterisation of bottleneck-aware scaling action selection (RQ2):
whether selecting compute or storage scale-out from a live observed bottleneck
improves service recovery and resource efficiency relative to fixed-priority
policies, under compute-bound and data-access-bound episodes.

\item The characterisation of readiness propagation (RQ3): how direct lifecycle
notification versus periodic discovery affects the time until a ready backend
contributes usable capacity.

\item The synthesis of the three interfaces under a common workload and event
schema, reconstructing the demand-to-usable-capacity timeline and identifying
which interface dominates under which conditions.
\end{enumerate}

These contributions are claimed within the evaluated two-domain testbed and
workload families; they do not assert superiority of the platform over
...
methodology used to conduct the review, it examines the separation of monitoring, routing and
scaling into independent components, and surveys auto-scaling, load balancing on SDN,
monitoring and telemetry, and resource orchestration on SDN, concluding with the research gaps
that motivate this dissertation.
...
semantics (RQ1), bottleneck-aware scaling action selection (RQ2), and readiness propagation
and traffic admission (RQ3), followed by resource orchestration and scalability analysis, and a
...
% constant; demand model / 10-phase workload (thesis_overview.md §2). Metrics:
