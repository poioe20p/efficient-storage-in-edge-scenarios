...
and redirect traffic to it, and each of these steps introduces delay, or
can fail, either of which worsens resource orchestration and can, in turn,
lead to user service quality degradation.
...
bottleneck (compute or storage scale-out), and how ready capacity is
admitted to traffic, including the failure of that admission path
(readiness admission).
...
problem: the handoff delays that accumulate, and the failures that can
arise, at the interfaces between telemetry delivery, capacity action
selection, and traffic admission under varying demand shifts. The
investigation rests on the idea that
...
\begin{itemize}
    \item \textbf{RQ1} -- How does the way demand evidence is delivered to the
    controller affect what it observes about overload, how it responds by
    scaling, and the quality of service users experience during a demand shift?

    \item \textbf{RQ2} -- When a service is bottlenecked on compute or on data
    access, how does selecting the scale-out action that matches the observed
    bottleneck affect relief of the tier under pressure, service recovery, and
    resource use?

    \item \textbf{RQ3} -- When a newly provisioned backend becomes ready, how
    does the way it is admitted to traffic affect when it actually serves
    requests, and does faster admission carry a service-quality cost when
    spare capacity is low and the readiness signal fails?
\end{itemize}

    RQ1 compares event-preserving, delayed event-preserving, and
    latest-state delivery; RQ2 compares bottleneck-matched selection against
    fixed single-tier policies under compute-bound and data-access-bound
    demand; and RQ3 compares lifecycle-coupled with periodically reconciled
    admission, including the failure of the readiness signal under thin
    incumbent headroom. The experimental form of each question is specified in
    \ref{ch:results}.
...
\begin{enumerate}
    ...
    \item An experimental characterisation of readiness-admission
    coordination that quantifies, under lifecycle-coupled and periodically
    reconciled admission, the time from verified readiness to usable
    capacity, and the bounded reliability cost the faster mechanism incurs
    when the lifecycle event source fails (RQ3).
...
to traffic and its robustness cost (RQ3), followed by resource orchestration and
scalability analysis, and a synthesis across the three interfaces.
...
The 2017 bound applies to the primary searches; older seminal works were
admitted only when reached by citation chaining or when they meet the
citation threshold, both recorded among the inclusion criteria of
...
Where the field measures the full provisioning-to-serving span, it does
so for serverless functions, whose cold start is dominated by image pull
and runtime initialisation and is reported end to end as a single time
to first response \parencite{EdgeFaaSBench,ColdStartServerlessSurvey}.
In this thesis those aspects are simplified away, since the backends are
pre-built containers that start in under a second, with the aim of
focusing on the interval that remains between readiness and first served
traffic, which is the interval the review isolates next.
...
and serves traffic. Lifecycle-coupled admission and periodically reconciled
admission are the two operational realisations of that coordination
interface.

\subsection{Readiness-Admission Coordination}
    The two operational realisations, admitting a backend when its lifecycle
    event fires or admitting it when a periodic scan observes the same state,
    are not new to software-defined networking. The OpenFlow programming model
    already separates the two philosophies under different names: a reactive
    controller installs forwarding state only when the first packet of a flow
    arrives, whereas a proactive controller pre-installs rules ahead of
    traffic, trading per-flow setup latency against controller round-trips
    and flow-table occupancy
    \parencite{Fernandez2013ComparingOpenFlowControllerParadigms}. The
    parallel is illustrative rather than exact: reactive installation is
    triggered by traffic rather than by loss resilience, but it is the same
    dichotomy applied to a different layer.
    The closest measured precedent is a SmartNIC-based load balancer that
    scales network functions at the edge and measures the transition from a
    scale signal to rebalanced traffic, yet it reports that transition as a
    single combined process and does not isolate the post-readiness handoff
    \parencite{Ni2021SmartLB}.

    The network side of the boundary has a direct precedent. Xu et al. dock
    and undock Docker applications at the edge switch through SDN, so that a
    container's network attachment is itself a controller-programmed step
    \parencite{Xu2016SDNDockerAutoDockingUndocking}. Their mechanism shows
    that attachment is programmable, but it does not separate the interval in
    which a verified application-ready backend waits to be admitted from the
    provisioning and attachment steps that precede it, and it does not vary
    the admission mechanism while holding the readiness criterion and the
    selection function fixed.

    The reviewed literature leaves a second dimension unexamined. Shard
    Manager's ordered lifecycle sequence assumes reliable dissemination and
    drops no requests, the load-balancing surveys treat pool state as
    accurate and current, and the freshness remarks of the service-discovery
    surveys concern stale data rather than lost events. No reviewed work
    models what happens to a ready backend when the readiness event that
    would admit it is lost. The admission interface is therefore both a
    latency choice and a reliability choice: lifecycle-coupled admission
    realises capacity sooner, while periodically reconciled admission
    self-heals on every cycle, because a lost event leaves a ready backend
    dark until it is re-checked or abandoned. RQ3 characterises the two
    together, measuring the time from verified readiness to usable capacity
    and the service-quality consequence when the event path fails and
    incumbent headroom is thin. The question mirrors the one RQ1 poses for
    demand evidence at the opposite end of the chain: how state moves from
    known to acted upon across a control-loop boundary, and what the crossing
    costs when the state is delayed or lost.
...
to traffic contribution under controlled invariants, and none models the
failure of the notification path that would admit a ready backend. Finally,
orchestration platforms either leave these functions separated or coordinate
them by construction; neither mode varies one interface while holding the
others fixed.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.34\textwidth} p{0.58\textwidth}}
        ...
        How does readiness-admission coordination affect the time from
        verified readiness to usable capacity, and does its delay become a
        transient service-quality cost when the event path fails and
        incumbent headroom is thin? \\
        \bottomrule
    \end{tabular}
\end{table}
