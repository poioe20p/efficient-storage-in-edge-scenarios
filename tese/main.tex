...
gaps that motivate this dissertation. % TO REVIEW CONFIRM IS THE STRUCTURE STILL HOLDS
...
workflow. % TO REVIEW CONFIRM IS THE STRUCTURE STILL HOLDS
...
\ref{sec:lit_synthesis}. % TO-DO: Verified that the strucutre referenced still holds up
...
orchestration response. 
    ...
    The state of the art in this area is built around 
...
the consumers that make decisions. 
In orchestrators this pipeline is
...
runs a telemetry collector and a routing adjuster in one closed loop
over the wide area, showing that monitoring and control functions can
be combined in a single component, although the loop omits scaling
altogether
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.40\textwidth} p{0.16\textwidth} p{0.36\textwidth}}
        ...
        Hung et al. (2022) & Measured & Telemetry and routing combined
        in one component; scaling omitted \\
...
\parencite{Qu2018AutoScalingWebApplicationsClouds,Taherizadeh2017AutoScalingApplicationsEdgeComputing}.
    ...
    When SDN is considered, the traffic side is added, but the scaled
...
\parencite{Dogani2023AutoScalingTechniquesContainerCloud}. That
provision remains compute-side: the data tier is not part of the
action space the reviewed systems expose.
...
but the bottleneck it locates is a microservice and the action it takes
is a change in that microservice's replica count. When the limiting resource is
the data store rather than any microservice, locating the bottleneck is
not enough: the action must change kind. That case, in which the
database is the tier under pressure, is the one this dissertation
evaluates.
...
management efficiency. 
    ...
    The most recent survey by El Mrabet et
al. extends the classification across cloud, fog, and edge, categorising
the surveyed methods by their decision mechanism, scaling strategy, and
deployment layer; which kind of resource is added is not among the
...
with the other interfaces, in \ref{sec:lit_three_layer_separation}.
...
decision can select it. In the papers studied here this sequence 
is not highlighted. Readiness
is a property of the running application rather than of the container that
hosts it, so a backend that has been provisioned and started may still be
unable to serve. The change of the eligible set is therefore gated by an
observation of that ability, and the interval between verified readiness
and routing eligibility is an admission delay, not a property of the
...
reducing over-provisioning and SLO violations incurred during provisioning
delays} \parencite{Bogdanov2018Kurma}. The delay between provisioning
and usable capacity is therefore a recognised cost, but the unit of
analysis remains the load-balancing service, and the interval in which
readiness state propagates to the routing plane is not separated from
it.

    Llorens-Carrodeguas et al. combine, through an SDN redirector,
monitoring, auto-scaling, and load balancing for a transparent VNF
cluster, activating and deactivating cluster members while the load
balancer keeps distributing traffic, and removing flow rules for
unhealthy instances using status obtained through the MANO monitoring
path
\parencite{Llorens2021SDNBasedHorizontalAutoScalingLoadBalancing}.
...
broader membership transitions. But none of the reviewed papers
isolates the interval between a backend becoming ready
and traffic actually reaching it. In this dissertation
evaluates that interval. It varies only how readiness admission is
coordinated, and it measures the time until newly provisioned capacity
becomes selectable and serves traffic. Direct lifecycle notification and
periodic discovery are the two mechanisms compared.
...
\ref{sec:platform_background} are assembled to serve. 
That decision-making, and its execution, is
...
gap as defined in \ref{sec:context_motivation}.
    ...
    Within the SDN edge resource orchestration literature, 
Baktir et al. organizes the gains that SDN brings to
...
services, treating contextual information such as data freshness as
part of the orchestration input
\parencite{Sofia2023DynamicContextAwareCrossLayer}.

    The surveyed architectures bind these vertical layers
into concrete deployments. Okwuibe et al. assemble the 
architecture closest to the one built in this
...
\parencite{Peng2019SDNResourceManagementAutonomousVehicular}.
    ...
    A recurring minority of these platforms brings the functions together
...
remain external
\parencite{Llorens2021SDNBasedHorizontalAutoScalingLoadBalancing};
Hung et al. present an SDN controller for 5G backhaul in which a
telemetry collector and routing adjustment run in one closed loop,
omitting scaling altogether
...
components. Combining the functions at an SDN control point is
therefore demonstrated as reviewed feasible, precedented arrangement.
...
on client QoS parameters, so the tracking of QoS and the customisation
of scheduling logic must be performed by an external component, and
that challenges remain to fully adapt these cloud-designed tools to
dynamic edge environments
\parencite{Cilic2023PerformanceEvaluationContainerOrchestrationTools}.
In a second, recurring mode, the same functions are kept together
at an SDN-based control point or coordinated through shared state. What
...
in passing, their symptoms observed, and their study called for, yet
the reviewed work does not isolate how any one of them contributes to
the end-to-end realization of usable capacity
\parencite{Yaseen2025CountersTelemetrySurveyProgrammableNetwork}.
