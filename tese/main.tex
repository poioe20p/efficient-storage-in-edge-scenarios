...
service quality~\parencite{Qu2018AutoScalingWebApplicationsClouds,Llorens2021SDNBasedHorizontalAutoScalingLoadBalancing}.
...
Table~\ref{tab:admission_evidence} summarises the strength of the
evidence each of these works provides for the admission interface.

\begin{table}[htbp]
    \centering
    \caption{Evidence on ready-backend admission in the reviewed literature.}
    \label{tab:admission_evidence}
    \begin{tabular}{p{0.40\textwidth} p{0.16\textwidth} p{0.36\textwidth}}
        \toprule
        Work & Strength & What it shows \\
        \midrule
        Neghabi et al. (2018); Belgaum et al. (2020) & Surveyed & Load-balancing
        policies optimise the distribution over the eligible set; its
        membership is taken as given \\
        Kurma (Bogdanov et al., 2018) & Measured & Seconds-granularity operation
        beside elastic controllers; provisioning-delay stages named but
        combined \\
        Llorens-Carrodeguas et al. (2021) & Measured & Membership changed through
        the monitoring path; activation reported at integrated-system
        granularity \\
        Okwuibe et al. (2020) & Measured & Migration latency reported as a
        bundle; the service is reachable again only after a DNS update \\
        \bottomrule
    \end{tabular}
\end{table}
...
The same rule selects a replica-set member for the data path,
using the corresponding storage metrics (CPU utilisation, memory usage,
connections, replication lag, and hop distance).
...
the storage tier. Service scale-up applies the policy defined in
...
utilisation and latency signals.
...
evaluation outcome. The summary contains the utilisation and latency signals
...
(delayed event-preserving delivery), so that the effect of delayed
...
(Figure~\ref{fig:control_workflow}). A unit that does not reach readiness
within the configured bound is abandoned and torn down instead of being
admitted.
...
\begin{figure}[H]
    ...
    rather than by calling each other directly. The numbered stages match the
    chain in Figure~\ref{fig:demand_capacity_chain} and show the provisioning
